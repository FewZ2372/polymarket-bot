...
class TradingConfig:
    ...
    take_profit_low: float = 0.10      # Entry < 0.30 -> +10% (mas realista para arbitraje)
    take_profit_medium: float = 0.08   # Entry 0.30-0.70 -> +8%
    take_profit_high: float = 0.05      # Entry > 0.70 -> +5%
    
    # Take profit decreciente por tiempo
    tp_decay_enabled: bool = True
    tp_decay_hours: List[int] = field(default_factory=lambda: [6, 12, 24, 36])  # Más agresivo
    tp_decay_targets: List[float] = field(default_factory=lambda: [0.10, 0.08, 0.05, 0.03])  # Decay más rápido
...
class MegaBot:
    ...
    def __init__(self, config: Optional[TradingConfig] = None):
        ...
        log.info(f"[MegaBot] Starting fresh - start_time: {self.stats['start_time'].isoformat()}")
    
    def _load_state(self, load_positions: bool = False):
        """Carga estado previo."""
        state_file = Path('mega_bot_state.json')
        if state_file.exists() and load_positions:
            try:
                ...
                if load_positions:
                    ...
                    log.info(f"[MegaBot] Loaded {len(self.positions)} positions from state")
                else:
                    log.info(f"[MegaBot] State file exists but not loading (fresh start)")
            except Exception as e:
                log.error(f"[MegaBot] Error loading state: {e}")
        elif state_file.exists():
            log.info(f"[MegaBot] State file exists but starting fresh (no positions loaded)")
    ...
    def _save_state(self):
        ...
        with open('mega_bot_state.json', 'w') as f:
    ...
    async def scan(self) -> List[Opportunity]:
        ...
        log.info(f"\n{'='*60}")
        log.info(f"[MegaBot] SCAN #{self.stats['total_scans']} - {datetime.now().strftime('%H:%M:%S')}")
        log.info(f"{'='*60}")
        
        # 1. Obtener datos
        log.info("[MegaBot] Fetching market data...")
        ...
        log.info(f"[MegaBot] {len(filtered)} opportunities after filtering")
        ...
        # 5. Verificar exits
        log.info(f"[MegaBot] Checking exits for {len(self.positions)} open positions...")
        exits = self._check_exits()
        if exits:
            log.info(f"[MegaBot] Found {len(exits)} positions to close")
        for pos in exits:
    ...
    def _filter_opportunities(self, opportunities: List[Opportunity], markets: List[Market]) -> List[Opportunity]:
        """Filtra oportunidades según configuración."""
        filtered = []
        ...
        for opp in opportunities:
            ...
            # Filtro: días a resolución (solo para YES definitivos)
            # Si es una apuesta YES y resuelve en más de una semana, skip
            if opp.action.value in ['buy_yes', 'buy_all_yes']:
                days_to_resolution = opp.days_to_resolution
                if days_to_resolution is None and opp.market_id:
                    # Buscar mercado para obtener end_date
                ...
                if days_to_resolution is not None and days_to_resolution > 7:
                    # Más de una semana - skip para YES definitivos
                    continue
    ...
    async def _update_positions(self, markets: List[Market], events: List[Dict] = None):
        ...
        log.info(f"[MegaBot] Updating {len(self.positions)} positions ({multi_outcome_count} multi-outcome), found {len(event_markets_by_event_id)} events")
        ...
        for pos in self.positions:
            ...
            if pos.extra_data.get('is_multi_outcome'):
                ...
                if event_id in event_markets_by_event_id:
                    log.info(f"[MegaBot] Found event {event_id} for position {pos.id} ({pos.market_question[:40]}...)")
                    event_markets = event_markets_by_event_id[event_id]
                    log.info(f"[MegaBot] Event has {len(event_markets)} markets")
                    for m in event_markets:
                        ...
                        if 'outcomePrices' in m:
                            ...
                            try:
                                ...
                                log.debug(f"[MegaBot] Error parsing outcomePrices: {e}")
                                pass
                        elif 'yes_price' in m:
                        ...
                        if 0 < price < 1:
                            ...
                            valid_prices += 1
                            log.debug(f"[MegaBot] Added price {price:.4f} from market {m.get('id', 'unknown')} (side: {pos.side})")
                        else:
                            log.debug(f"[MegaBot] Invalid price {price} from market {m.get('id', 'unknown')}")
                    
                    log.info(f"[MegaBot] Extracted {valid_prices} valid prices from event {event_id}")
        ...
        if updated_count > 0:
            log.info(f"[MegaBot] Updated prices for {updated_count} positions")
    
    def _check_exits(self) -> List[Position]:
        """Verifica qué posiciones deben cerrarse."""
        exits = []
        ...
        for i, pos in enumerate(self.positions):
            ...
            if i < 3:
                log.info(f"[MegaBot] Checking exit #{i+1}: {pos.market_question[:40]}... | "
                        f"P&L: {current_pnl_pct:+.2%} | Target TP: {target_tp:+.2%} | "
                        f"Entry: ${pos.entry_price:.4f} | Current: ${pos.current_price:.4f} | "
                        f"Hours: {pos.hours_open:.1f}h | Should close: {current_pnl_pct >= target_tp}")
            
            # Take profit
            if pos.should_take_profit(self.config):
                log.info(f"[MegaBot] [OK] TAKE PROFIT TRIGGERED: {pos.market_question[:40]}... "
                        f"({current_pnl_pct:+.1%} >= {target_tp:+.1%})")
                exits.append(pos)
                continue
            
            # Stop loss
            if pos.should_stop_loss(self.config):
                log.info(f"[MegaBot] [X] STOP LOSS TRIGGERED: {pos.market_question[:40]}... "
                        f"({current_pnl_pct:+.1%})")
                exits.append(pos)
    ...
    def _close_position(self, pos: Position, exit_price: float, reason: str):
        ...
        log.info(f"[MegaBot] CLOSED: {reason} | PnL: ${pos.pnl:.2f} | "
    ...
    def _execute_trades(self, opportunities: List[Opportunity]) -> List[Position]:
        ...
        for opp in opportunities[:5]:  # Máximo 5 trades por scan
            ...
            if entry_price is None or entry_price <= 0 or entry_price >= 1.0:
                log.warning(f"[MegaBot] Invalid entry price {entry_price} for {opp.market_question[:40]}...")
                continue
            ...
            if opp.type.value == 'multi_outcome_arb' and opp.markets:
                ...
                if market_ids or condition_ids:
                    ...
                    log.info(f"[MegaBot] Multi-outcome position: {len(market_ids)} market IDs, {len(condition_ids)} condition IDs")
                else:
                    log.warning(f"[MegaBot] No individual market IDs found for multi-outcome arb")
            ...
            log.info(
                f"[MegaBot] NEW TRADE: {opp.type.value} | {side} @ ${entry_price:.2f} | "
...
async def main():
    ...
    if not args.no_server:
        ...
        server_thread.start()
        log.info(f"[MegaBot] Dashboard running at http://localhost:{args.port}")
